...
def define_G_A(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    net = MaskNoiseNet(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
...
class MaskNoiseNet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        """
        assert(n_blocks >= 0)
        super(MaskNoiseNet, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d

        down_model = [nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            down_model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        ...
        up_model_mask += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        up_model_mask += [nn.Sigmoid()]

        self.up_model_mask = nn.Sequential(*up_model_mask)
        ...
        up_model_noise += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        up_model_noise += [nn.Tanh()]

        self.up_model_noise = nn.Sequential(*up_model_noise)

    def forward(self, input):
        middle = self.down_model(input)
        mask = self.up_model_mask(middle)
        noise = self.up_model_noise(middle)

        bin_mask = (mask < 0.5).float() * 1.

        noise = noise + input[:, 0:1, :, :]
        output_noise = bin_mask * noise

        output = torch.cat((output_noise, mask), dim=1)
        return output
